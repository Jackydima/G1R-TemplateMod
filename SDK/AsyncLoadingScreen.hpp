#ifndef UE4SS_SDK_AsyncLoadingScreen_HPP
#define UE4SS_SDK_AsyncLoadingScreen_HPP

#include "AsyncLoadingScreen_enums.hpp"

struct FALoadingScreenSettings
{
    float MinimumLoadingScreenDisplayTime;                                            // 0x0000 (size: 0x4)
    bool bAutoCompleteWhenLoadingCompletes;                                           // 0x0004 (size: 0x1)
    bool bMoviesAreSkippable;                                                         // 0x0005 (size: 0x1)
    bool bWaitForManualStop;                                                          // 0x0006 (size: 0x1)
    bool bAllowInEarlyStartup;                                                        // 0x0007 (size: 0x1)
    bool bAllowEngineTick;                                                            // 0x0008 (size: 0x1)
    TEnumAsByte<EMoviePlaybackType> PlaybackType;                                     // 0x0009 (size: 0x1)
    TArray<FString> MoviePaths;                                                       // 0x0010 (size: 0x10)
    bool bShuffle;                                                                    // 0x0020 (size: 0x1)
    bool bSetDisplayMovieIndexManually;                                               // 0x0021 (size: 0x1)
    bool bShowWidgetOverlay;                                                          // 0x0022 (size: 0x1)
    bool bShowLoadingCompleteText;                                                    // 0x0023 (size: 0x1)
    bool bShowLoadingWidget;                                                          // 0x0024 (size: 0x1)
    bool bUseRenderedFrameAsBackground;                                               // 0x0025 (size: 0x1)
    int32 OverrideSyncIntervalForMovies;                                              // 0x0028 (size: 0x4)
    TArray<FKey> SkipKeys;                                                            // 0x0030 (size: 0x10)
    double HoldToSkipDuration;                                                        // 0x0040 (size: 0x8)
    TSoftObjectPtr<UMaterial> HoldToSkipProgressMaterial;                             // 0x0048 (size: 0x28)
    FName HoldToSkipProgressMaterialParameter;                                        // 0x0070 (size: 0x8)
    FVector2f HoldToSkipSize;                                                         // 0x0078 (size: 0x8)
    FText HoldToSkipText;                                                             // 0x0080 (size: 0x10)
    FTextAppearance HoldToSkipTextAppearance;                                         // 0x0090 (size: 0xA0)
    FLoadingCompleteTextSettings LoadingCompleteTextSettings;                         // 0x0130 (size: 0xD0)
    FBackgroundSettings Background;                                                   // 0x0200 (size: 0x48)
    FTipSettings TipWidget;                                                           // 0x0248 (size: 0x178)
    TArray<FBackgroundSettings> StoryRecapBackgrounds;                                // 0x03C0 (size: 0x10)
    TArray<FTipSettings> StoryRecapTipWidgets;                                        // 0x03D0 (size: 0x10)
    FLoadingWidgetSettings LoadingWidget;                                             // 0x03E0 (size: 0x2F0)
    EAsyncLoadingScreenLayout Layout;                                                 // 0x06D0 (size: 0x1)

}; // Size: 0x6E0

struct FALoadingScreenSettings_Overwrite
{
    FALoadingScreenSettings ScreenSettings;                                           // 0x0000 (size: 0x6E0)
    FClassicLayoutSettings Classic;                                                   // 0x06E0 (size: 0xF0)
    FCenterLayoutSettings Center;                                                     // 0x07D0 (size: 0xF0)
    FLetterboxLayoutSettings Letterbox;                                               // 0x08C0 (size: 0x1F0)
    FSidebarLayoutSettings Sidebar;                                                   // 0x0AB0 (size: 0x100)
    FDualSidebarLayoutSettings DualSidebar;                                           // 0x0BB0 (size: 0x1D0)
    class UTexture2D* SaveTextureForLoadingScreen;                                    // 0x0D80 (size: 0x8)

}; // Size: 0xD90

struct FBackgroundSettings
{
    TArray<FSoftObjectPath> Images;                                                   // 0x0000 (size: 0x10)
    TArray<TSoftObjectPtr<UTexture2D>> StoryImages;                                   // 0x0010 (size: 0x10)
    TEnumAsByte<EStretch::Type> ImageStretch;                                         // 0x0020 (size: 0x1)
    FMargin Padding;                                                                  // 0x0024 (size: 0x10)
    FLinearColor BackgroundColor;                                                     // 0x0034 (size: 0x10)
    bool bSetDisplayBackgroundManually;                                               // 0x0044 (size: 0x1)

}; // Size: 0x48

struct FCenterLayoutSettings
{
    bool bIsTipAtBottom;                                                              // 0x0000 (size: 0x1)
    FWidgetAlignment TipAlignment;                                                    // 0x0001 (size: 0x2)
    TEnumAsByte<EHorizontalAlignment> BorderHorizontalAlignment;                      // 0x0003 (size: 0x1)
    float BorderVerticalOffset;                                                       // 0x0004 (size: 0x4)
    FMargin BorderPadding;                                                            // 0x0008 (size: 0x10)
    FSlateBrush BorderBackground;                                                     // 0x0020 (size: 0xD0)

}; // Size: 0xF0

struct FCircularThrobberSettings
{
    int32 NumberOfPieces;                                                             // 0x0000 (size: 0x4)
    float Period;                                                                     // 0x0004 (size: 0x4)
    float Radius;                                                                     // 0x0008 (size: 0x4)
    FSlateBrush Image;                                                                // 0x0010 (size: 0xD0)

}; // Size: 0xE0

struct FClassicLayoutSettings
{
    bool bIsWidgetAtBottom;                                                           // 0x0000 (size: 0x1)
    bool bIsLoadingWidgetAtLeft;                                                      // 0x0001 (size: 0x1)
    float Space;                                                                      // 0x0004 (size: 0x4)
    FWidgetAlignment TipAlignment;                                                    // 0x0008 (size: 0x2)
    TEnumAsByte<EHorizontalAlignment> BorderHorizontalAlignment;                      // 0x000A (size: 0x1)
    FMargin BorderPadding;                                                            // 0x000C (size: 0x10)
    FSlateBrush BorderBackground;                                                     // 0x0020 (size: 0xD0)

}; // Size: 0xF0

struct FDualSidebarLayoutSettings
{
    bool bIsLoadingWidgetAtRight;                                                     // 0x0000 (size: 0x1)
    TEnumAsByte<EVerticalAlignment> LeftVerticalAlignment;                            // 0x0001 (size: 0x1)
    TEnumAsByte<EVerticalAlignment> RightVerticalAlignment;                           // 0x0002 (size: 0x1)
    TEnumAsByte<EVerticalAlignment> LeftBorderVerticalAlignment;                      // 0x0003 (size: 0x1)
    TEnumAsByte<EVerticalAlignment> RightBorderVerticalAlignment;                     // 0x0004 (size: 0x1)
    FMargin LeftBorderPadding;                                                        // 0x0008 (size: 0x10)
    FMargin RightBorderPadding;                                                       // 0x0018 (size: 0x10)
    FSlateBrush LeftBorderBackground;                                                 // 0x0030 (size: 0xD0)
    FSlateBrush RightBorderBackground;                                                // 0x0100 (size: 0xD0)

}; // Size: 0x1D0

struct FImageSequenceSettings
{
    TArray<class UTexture2D*> Images;                                                 // 0x0000 (size: 0x10)
    FVector2D Scale;                                                                  // 0x0010 (size: 0x10)
    float Interval;                                                                   // 0x0020 (size: 0x4)
    bool bPlayReverse;                                                                // 0x0024 (size: 0x1)

}; // Size: 0x28

struct FLetterboxLayoutSettings
{
    TEnumAsByte<EVerticalAlignment> VerticalLoadingWidgetPosition;                    // 0x0000 (size: 0x1)
    TEnumAsByte<EVerticalAlignment> VerticalTipWidgetPosition;                        // 0x0001 (size: 0x1)
    FMargin TipWidgetPositionPadding;                                                 // 0x0004 (size: 0x10)
    FMargin LoadingWidgetPositionPadding;                                             // 0x0014 (size: 0x10)
    FWidgetAlignment TipAlignment;                                                    // 0x0024 (size: 0x2)
    FWidgetAlignment LoadingWidgetAlignment;                                          // 0x0026 (size: 0x2)
    TEnumAsByte<EHorizontalAlignment> TopBorderHorizontalAlignment;                   // 0x0028 (size: 0x1)
    TEnumAsByte<EHorizontalAlignment> BottomBorderHorizontalAlignment;                // 0x0029 (size: 0x1)
    FMargin TopBorderPadding;                                                         // 0x002C (size: 0x10)
    FMargin BottomBorderPadding;                                                      // 0x003C (size: 0x10)
    FSlateBrush TopBorderBackground;                                                  // 0x0050 (size: 0xD0)
    FSlateBrush BottomBorderBackground;                                               // 0x0120 (size: 0xD0)

}; // Size: 0x1F0

struct FLoadingCompleteTextSettings
{
    FText LoadingCompleteText;                                                        // 0x0000 (size: 0x10)
    FTextAppearance Appearance;                                                       // 0x0010 (size: 0xA0)
    FWidgetAlignment Alignment;                                                       // 0x00B0 (size: 0x2)
    FMargin Padding;                                                                  // 0x00B4 (size: 0x10)
    bool bFadeInOutAnim;                                                              // 0x00C4 (size: 0x1)
    float AnimationSpeed;                                                             // 0x00C8 (size: 0x4)

}; // Size: 0xD0

struct FLoadingWidgetSettings
{
    ELoadingIconType LoadingIconType;                                                 // 0x0000 (size: 0x1)
    ELoadingWidgetType LoadingWidgetType;                                             // 0x0001 (size: 0x1)
    FVector2D TransformTranslation;                                                   // 0x0008 (size: 0x10)
    FVector2D TransformScale;                                                         // 0x0018 (size: 0x10)
    FVector2D TransformPivot;                                                         // 0x0028 (size: 0x10)
    FText LoadingText;                                                                // 0x0038 (size: 0x10)
    bool bLoadingTextRightPosition;                                                   // 0x0048 (size: 0x1)
    bool bLoadingTextTopPosition;                                                     // 0x0049 (size: 0x1)
    FTextAppearance Appearance;                                                       // 0x0050 (size: 0xA0)
    FThrobberSettings ThrobberSettings;                                               // 0x00F0 (size: 0xE0)
    FCircularThrobberSettings CircularThrobberSettings;                               // 0x01D0 (size: 0xE0)
    FImageSequenceSettings ImageSequenceSettings;                                     // 0x02B0 (size: 0x28)
    FWidgetAlignment TextAlignment;                                                   // 0x02D8 (size: 0x2)
    FWidgetAlignment LoadingIconAlignment;                                            // 0x02DA (size: 0x2)
    float Space;                                                                      // 0x02DC (size: 0x4)
    bool bHideLoadingWidgetWhenCompletes;                                             // 0x02E0 (size: 0x1)

}; // Size: 0x2F0

struct FSidebarLayoutSettings
{
    bool bIsWidgetAtRight;                                                            // 0x0000 (size: 0x1)
    bool bIsLoadingWidgetAtTop;                                                       // 0x0001 (size: 0x1)
    float Space;                                                                      // 0x0004 (size: 0x4)
    TEnumAsByte<EVerticalAlignment> VerticalAlignment;                                // 0x0008 (size: 0x1)
    FWidgetAlignment LoadingWidgetAlignment;                                          // 0x0009 (size: 0x2)
    FWidgetAlignment TipAlignment;                                                    // 0x000B (size: 0x2)
    TEnumAsByte<EVerticalAlignment> BorderVerticalAlignment;                          // 0x000D (size: 0x1)
    float BorderHorizontalOffset;                                                     // 0x0010 (size: 0x4)
    FMargin BorderPadding;                                                            // 0x0014 (size: 0x10)
    FSlateBrush BorderBackground;                                                     // 0x0030 (size: 0xD0)

}; // Size: 0x100

struct FTextAppearance
{
    FSlateColor ColorAndOpacity;                                                      // 0x0000 (size: 0x14)
    FSlateFontInfo Font;                                                              // 0x0018 (size: 0x60)
    FVector2D ShadowOffset;                                                           // 0x0078 (size: 0x10)
    FLinearColor ShadowColorAndOpacity;                                               // 0x0088 (size: 0x10)
    TEnumAsByte<ETextJustify::Type> Justification;                                    // 0x0098 (size: 0x1)

}; // Size: 0xA0

struct FThrobberSettings
{
    int32 NumberOfPieces;                                                             // 0x0000 (size: 0x4)
    bool bAnimateHorizontally;                                                        // 0x0004 (size: 0x1)
    bool bAnimateVertically;                                                          // 0x0005 (size: 0x1)
    bool bAnimateOpacity;                                                             // 0x0006 (size: 0x1)
    FSlateBrush Image;                                                                // 0x0010 (size: 0xD0)

}; // Size: 0xE0

struct FTipSettings
{
    TArray<FText> TipTextHeadLine;                                                    // 0x0000 (size: 0x10)
    FTextAppearance AppearanceHeadline;                                               // 0x0010 (size: 0xA0)
    TArray<FText> TipText;                                                            // 0x00B0 (size: 0x10)
    int32 NumberOfLines;                                                              // 0x00C0 (size: 0x4)
    float TipSwapTime;                                                                // 0x00C4 (size: 0x4)
    bool bSupportsSubtitles;                                                          // 0x00C8 (size: 0x1)
    float VoiceOverDelay;                                                             // 0x00CC (size: 0x4)
    FTextAppearance Appearance;                                                       // 0x00D0 (size: 0xA0)
    float TipWrapAt;                                                                  // 0x0170 (size: 0x4)
    bool bSetDisplayTipTextManually;                                                  // 0x0174 (size: 0x1)

}; // Size: 0x178

struct FWidgetAlignment
{
    TEnumAsByte<EHorizontalAlignment> HorizontalAlignment;                            // 0x0000 (size: 0x1)
    TEnumAsByte<EVerticalAlignment> VerticalAlignment;                                // 0x0001 (size: 0x1)

}; // Size: 0x2

class UAsyncLoadingScreenLibrary : public UBlueprintFunctionLibrary
{

    void StopLoadingScreen();
    void SetEnableLoadingScreen(bool bIsEnableLoadingScreen);
    void SetDisplayTipTextIndex(int32 TipTextIndex);
    void SetDisplayMovieIndex(int32 MovieIndex);
    void SetDisplayBackgroundIndex(int32 BackgroundIndex);
    void RemovePreloadedBackgroundImages();
    void PreloadBackgroundImages();
    bool GetIsEnableLoadingScreen();
}; // Size: 0x28

class UAsyncLoadingScreen_Input : public UObject
{

    bool SetupThrobberImage(FALoadingScreenSettings_Overwrite& _LoadingScreenSettings, FSlateBrush& _TargetImageOfSettings, FString _TexturePath, FVector2D _Size);
    bool SetupLoadingscreenSettings(FALoadingScreenSettings_Overwrite& _LoadingScreenSettings);
}; // Size: 0x28

class ULoadingScreenSettings : public UDeveloperSettings
{
    bool bPreloadBackgroundImages;                                                    // 0x0038 (size: 0x1)
    FALoadingScreenSettings StartupLoadingScreen;                                     // 0x0040 (size: 0x6E0)
    FALoadingScreenSettings DefaultLoadingScreen;                                     // 0x0720 (size: 0x6E0)
    TSoftClassPtr<UAsyncLoadingScreen_Input> OverwriteLoadingscreenScript;            // 0x0E00 (size: 0x28)
    TSoftClassPtr<UAsyncLoadingScreen_Input> OverwriteStreamingLoadingScreenScript;   // 0x0E28 (size: 0x28)
    FClassicLayoutSettings Classic;                                                   // 0x0E50 (size: 0xF0)
    FCenterLayoutSettings Center;                                                     // 0x0F40 (size: 0xF0)
    FLetterboxLayoutSettings Letterbox;                                               // 0x1030 (size: 0x1F0)
    FSidebarLayoutSettings Sidebar;                                                   // 0x1220 (size: 0x100)
    FDualSidebarLayoutSettings DualSidebar;                                           // 0x1320 (size: 0x1D0)

}; // Size: 0x14F0

#endif
